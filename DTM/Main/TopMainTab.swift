...
struct TopMainTab: View {
    ...
    @State var doubleData = ["", ""]
    @State private var isDoubleMatch = false
    ...
    var body: some View {
        VStack(spacing: 0) {
            TabView(selection: $selection) {
                if !isDoubleMatch {
                    TopDoubleDatesView(data: $doubleData)
                        .tabItem {
                        ...
                        .task {
                            Task {
                                do {
                                    try await getDouble(doubleData: $doubleData)
                                    try await checkDoubleMatch(name: userName) { judge in
                                        if judge {
                                            isDoubleMatch = true
                                        } else {
                                            isDoubleMatch = false
