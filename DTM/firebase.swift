...
func getDouble(doubleData: Binding<[String]>) async throws {
    let db = Firestore.firestore()
        ...
        if document.exists {
            if let data = document.data() {
                ...
                if let maxKey = maxKey, let maxData = data["\(maxKey)"] as? [Any] {
                    ...
                    if let firstElement = maxData[0] as? String, let secondElement = maxData[1] as? String {
                        print("1つ目の要素: \(firstElement)")
                        print("2つ目の要素: \(secondElement)")
                        doubleData[0].wrappedValue = firstElement
                        doubleData[1].wrappedValue = secondElement
                    } else {
...
// DoubleDatesページを開いたときに、自分がDoubleDatesの申込みをしているか確認
func checkSend(name: String, completion: @escaping (String) -> Void) {
    let db = Firestore.firestore()
    let ref = db.collection("doubledates")
    // ドキュメントを取得
    ref.getDocuments { (querySnapshot, error) in
        ...
        // 各ドキュメントの`from`フィールド内に`name`があるかを確認
        for document in documents {
            print(document)
            let data = document.data()
            print(data)
            // `from`フィールドが存在し、String型か確認
            if let fromField = data["from"] as? String, fromField == name {
                completion(document.documentID)  // `from`フィールドに`name`がある
                return
            }
        }
        // `from`フィールド内に`name`が見つからない場合
        completion("none")
    }
}
            ...
            if let data = document.data() {
                ...
                if let maxKey = maxKey, let maxData = data["\(maxKey)"] as? [Any] {
                    ...
                    if let thirdElement = maxData[2] as? [String: Any] {
                        if var stateArray = thirdElement["state"] as? [String] {
                            // 既存のstate配列に新しいペアを追加
                            let newPair = "\(name1)&\(name2)"
                            stateArray.append(newPair)  // 新しいペアを追加
                            
                            // Firestoreに配列全体を上書き保存
                            var updatedMaxData = maxData
                            var updatedThirdElement = thirdElement
                            updatedThirdElement["state"] = stateArray
                            updatedMaxData[2] = updatedThirdElement
                            
                            try await ref.updateData([
                                "\(maxKey)": updatedMaxData
                            ])
